            ...
            if (data) {
                ...
                if (perm.geography) {
                    try {
                        ...
                        } else if (Array.isArray(parsedConstraint)) {
                             // If it's a JSON array, check if territory or country is exactly in the array
                             if (!parsedConstraint.includes(data._geography_territory) && !parsedConstraint.includes(data.country)) {
                                 matches = false;
                             }
                        } else {
                             // Fallback to strict string matching if it's not a known structure
                             if (data._geography_territory !== perm.geography && data.country !== perm.geography) {
                                 matches = false;
                             }
                        }
                    } catch (e) {
                         // Not JSON, fallback to exact matching of comma separated values or string
                         const allowedGeographies = perm.geography.split(',').map((g: string) => g.trim());
                         if (!allowedGeographies.includes(data._geography_territory) && !allowedGeographies.includes(data.country)) {
                             matches = false;
                         }
                    }
                }
