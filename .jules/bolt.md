...
## 2024-05-02 - [Backend Controllers] Cache Repetitive JSON.parse on Identical Strings
**Learning:** In backend controllers (`EntityConfigController`, `ConfigController`), performing `JSON.parse` on identical string fields (like `defaultValues` or `listSubFields` across presets or across requests) inside mapping loops introduces unnecessary O(N) CPU overhead on the main thread.
**Action:** Implemented a generic `Map<string, any>` cache in the backend `utils.ts` (`cachedJsonParse`) keyed by the exact JSON string. Replaced inline `JSON.parse` calls to change complexity from O(N) repetitive parsing to O(1) cache retrieval per unique JSON string.
