...
## 2024-05-18 - Information Leakage in API Responses
**Vulnerability:** Found multiple Express controllers (like `data-source.controller.ts` and `entity-config.controller.ts`) directly passing the raw `e.message` or downstream internal error payloads (`e.response?.data?.errors`) to the client via 500 status code responses. This leaks internal implementation details, downstream errors, or stack traces which can be useful to an attacker to understand the backend architecture or perform targeted attacks.
**Learning:** Developers often pass `e.message` in error responses to provide better debugging context on the frontend, but this violates security best practices around failing securely.
**Prevention:** To prevent information leakage, backend controllers must not expose raw error messages or downstream error payloads in HTTP 500 responses. We must log the detailed error internally using `console.error` and return a generic, static error message (e.g., `{ error: 'An internal server error occurred' }`).
