...
export class EntityConfigController {
    ...
    getConfigById = async (req: Request, res: Response) => {
        try {
            ...
            console.error(`Error getting config ${req.params.id}:`, e.response?.data?.errors ? JSON.stringify(e.response.data.errors) : e.message);
    ...
    createConfig = async (req: Request, res: Response) => {
        try {
            ...
            console.error('Error creating config:', e.response?.data?.errors ? JSON.stringify(e.response.data.errors) : e.message);
            res.status(500).json({ error: 'An internal server error occurred' });
        }
    };
    ...
    updateConfig = async (req: Request, res: Response) => {
        try {
            ...
            console.error(`Error updating config ${req.params.id}:`, e.response?.data?.errors ? JSON.stringify(e.response.data.errors) : e.message);
            res.status(500).json({ error: 'An internal server error occurred' });
        }
    };
    ...
    deleteConfig = async (req: Request, res: Response) => {
        try {
            ...
            console.error(`Error deleting config ${req.params.id}:`, e.response?.data?.errors ? JSON.stringify(e.response.data.errors) : e.message);
            res.status(500).json({ error: 'An internal server error occurred' });
        }
    };
    ...
    introspect = async (req: Request, res: Response) => {
        try {
            ...
            console.error('Error in introspect:', e.response?.data?.errors ? JSON.stringify(e.response.data.errors) : e.message);
            res.status(500).json({ error: 'An internal server error occurred' });
        }
    };
}
