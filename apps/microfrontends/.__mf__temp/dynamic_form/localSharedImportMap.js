...
// Windows temporarily needs this file, https://github.com/module-federation/vite/issues/68
    ...
    const importMap = {
        ...
        "axios": async () => {
          let pkg = await import("__mf__virtual/dynamic_form__prebuild__axios__prebuild__.js");
            return pkg;
        }
      ,
        "react": async () => {
          let pkg = await import("__mf__virtual/dynamic_form__prebuild__react__prebuild__.js");
            return pkg;
        }
      ,
        "react-dom": async () => {
          let pkg = await import("__mf__virtual/dynamic_form__prebuild__react_mf_2_dom__prebuild__.js");
            return pkg;
        }
      ,
        "react-router-dom": async () => {
          let pkg = await import("__mf__virtual/dynamic_form__prebuild__react_mf_2_router_mf_2_dom__prebuild__.js");
            return pkg;
        }
      ...
      const usedShared = {
          ...
          "axios": {
            ...
            async get () {
              if (false) {
                throw new Error(`[Module Federation] Shared module '${"axios"}' must be provided by host`);
              }
              usedShared["axios"].loaded = true
              const {"axios": pkgDynamicImport} = importMap
              const res = await pkgDynamicImport()
              const exportModule = false && "axios" === "react"
                ? (res?.default ?? res)
                : {...res}
