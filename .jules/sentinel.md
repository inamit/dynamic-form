...
## 2024-05-30 - [Information Leakage in Error Responses]
**Vulnerability:** Downstream error payloads and stack traces (e.g. `e.response?.data?.errors`, `e.message`) were being sent directly to the client in `res.status(500).json({ error: ... })` within multiple controllers (e.g., `DataSourceController`, `EntityConfigController`, `DataController`). This violates the "Fail Securely" principle and can expose internal architecture, downstream dependencies, and sensitive operational data to potential attackers.
**Learning:** Returning raw error data from APIs is a common source of information leakage. Client-facing error messages should be generic while detailed errors are logged securely on the backend.
**Prevention:** Catch blocks handling internal server errors (500) should log the detailed error using `console.error` (for server-side monitoring) and respond with a generic message like `{ error: 'An internal server error occurred' }`.
